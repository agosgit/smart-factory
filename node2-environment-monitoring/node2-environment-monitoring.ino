...
float readTemperature() {
  ...
  return random(240, 350) / 10.0;
#else
  // Ganti dengan pembacaan sensor DHT22 nyata
  return 0.0;
#endif
}
...
float readGasLevel() {
#if SIMULATE_SENSOR_DATA
  return random(80, 500);
#else
  // Ganti dengan pembacaan sensor MQ-2 nyata
  return 0.0;
#endif
}
...
void publishTelemetry() {
  ...
  if (mqttClient.publish(TELEMETRY_TOPIC, payload.c_str())) {
    Serial.println("Published Node 2 telemetry:");
    Serial.println(payload);
    blinkLED();
  } else {
    Serial.println("Publish failed for Node 2 telemetry.");
  }
}
...
void setup() {
  ...
  Serial.println("Node 2 Environment Monitoring initialized.");
